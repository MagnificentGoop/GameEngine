...
void Player::Update() {
    ...
    if (bad::g_engine.GetInput().GetKeyPressed(SDL_SCANCODE_W)) AddVelocity(velocity);
    if (bad::g_engine.GetInput().GetKeyPressed(SDL_SCANCODE_S)) AddVelocity(bad::Vector2<float>{-1,-1} * velocity);
